...
Because tick splitting introduces that error for accelerating gamemodes, Superb Input Precision takes a different approach. Vanilla still owns the physics and runs its normal ticks, but this mod applies a Y-displacement correction so that an input applied partway through a tick produces the position vanilla would have produced if inputs were processed at that moment.
